...
public class FlyingHouseManager : MonoBehaviour
{
    ...
    public double startLongitude = -74.006f;
    ...
    {
        if (windAudio != null) { windAudio.loop = true; windAudio.Play(); }
        if (ambienceAudio != null) { ambienceAudio.loop = true; ambienceAudio.Play(); }
    }
    
    void Update() => UpdateWindAudio();
    ...
    public void GoToLocation(string locationName)
    {
        ...
        {
            case "nyc": case "new york":
                flightController.ResetPosition(-74.006, 40.7128, 500); break;
            case "sf": case "san francisco":
                flightController.ResetPosition(-122.4194, 37.7749, 500); break;
            case "paris":
                flightController.ResetPosition(2.3522, 48.8566, 500); break;
            case "tokyo":
                flightController.ResetPosition(139.6917, 35.6895, 500); break;
            default:
                Debug.Log("Unknown location: " + locationName); break;
        }
    }
    
    public void PauseFlight() { if (flightController != null) flightController.SetFlying(false); }
    public void ResumeFlight() { if (flightController != null) flightController.SetFlying(true); }
}
